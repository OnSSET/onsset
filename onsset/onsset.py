...
class SettlementProcessor:
    ...
    def elec_current_and_future(self, elec_actual, elec_actual_urban, elec_actual_rural, start_year,
        ...
        # Update which settlements are considered to be electrified
        ...
        urban_elec_modelled = self.df.loc[(self.df[SET_ELEC_CURRENT] == 1) & (self.df[SET_URBAN] > 1),
                                          SET_ELEC_POP_CALIB].sum()

        rural_elec_modelled = self.df.loc[(self.df[SET_ELEC_CURRENT] == 1) & (self.df[SET_URBAN] <= 1),
                                          SET_ELEC_POP_CALIB].sum()
        ...
        # REVIEW. If not enough people live where there is NTL, electrify people around buffer from existing grid
        i = 0
        td_dist_2 = 0.1
        ...
        print('The modelled electrification rate differ by {0:.2f}. '
            ...
            elec_modelled - elec_actual, elec_modelled_urban - elec_actual_urban,
            elec_modelled_rural - elec_actual_rural))
    ...
    def pre_electrification(self, grid_price, year, time_step, end_year, grid_calc, grid_capacity_limit,
        ...
        electrified_loce, electrified_investment, electrified_capacity = self.get_grid_lcoe(0, 0, 0, year, time_step, end_year, grid_calc)
    ...
    def elec_extension(self, grid_calc, max_dist, year, start_year, end_year, time_step, grid_capacity_limit,
        ...
        return new_lcoes, cell_path_adjusted, elecorder, cell_path_real, pd.DataFrame(new_investment), pd.DataFrame(new_capacity)
